...
@interface WOTWXApiRequestHandler()

@property (nonatomic, copy) NSString *timeStamp;
@property (nonatomic, copy) NSString *nonceStr;
@property (nonatomic, copy) NSString *traceId;
@end
@implementation WOTWXApiRequestHandler
...
+(void)sendWXpay{
    ...
    [WOTHttp doRequestWithParameters:paramdic useUrl:PaySERVER_URL complete:^JSONModel *(id responseobj) {
        ...
        payRequest.partnerId = paymodel.msg.mch_id;//商户id
...
+(void)sendWXpayWithServerWithMoney:(NSString *)money{
    ...
    NSLog(@"%@", [postDict objectForKey:@"notify_url"]);
...
+(NSString *)genTraceId
{
    ...
    //    return [NSString stringWithFormat:@"%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"OUT_TRADE_NO"]];
}
...
+(NSString *)genSign:(NSDictionary *)signParams
{
    ...
    NSString *result = [[WOTMD5 md5:sign] uppercaseString];
...
// 构造预付订单参数列表
+(NSDictionary *)getProductArgsWithMoney:(NSString *)money
{
    ...
    //    NSString *outTradNo = [[NSUserDefaults standardUserDefaults] objectForKey:@"OUT_TRADE_NO"];
    
    //    NSString * out_trade_no= [NSString stringWithFormat:@"%@%@",timeStamp,outTradNo];//获取商户订单号
    NSString * out_trade_no= [[NSUserDefaults standardUserDefaults] objectForKey:@"OUT_TRADE_NO"];//获取商户订单号
    ...
    [params setObject:@"易创客" forKey:@"body"];//商品描述，必填
    ...
    [params setObject:[WOTMD5 getIPAddress:YES] forKey:@"spbill_create_ip"];//终端ip，必填
    
    NSString * total_fee = [NSString stringWithFormat:@"%.0f", [money doubleValue] * 100];
    NSLog(@"%@", total_fee);
